...
public class AdachireiNuiBlockEntityRenderer implements BlockEntityRenderer<AdachireiNuiBlockEntity, AdachireiNuiRenderState> {
    ...
    private static Identifier modelIdFor(Block block) {
        if (block == AdachireiNui.ADACHI_BLOCK_5) return MODEL_ADACHI_NUI;
        if (block == AdachireiNui.ADACHI_BLOCK_6) return MODEL_ADACHI_NUI_NEW;
        if (block == AdachireiNui.ADACHI_BLOCK_7) return MODEL_ADACHI_NUI_VOCALOID;
        return null;
    }
    ...
    private static void rotateVector(Vector3f vector, Axis axis, float sin, float cos) {
        ...
        switch (axis) {
            case X -> { vector.y = y * cos - z * sin; vector.z = y * sin + z * cos; }
            case Y -> { vector.x = x * cos + z * sin; vector.z = -x * sin + z * cos; }
            case Z -> { vector.x = x * cos - y * sin; vector.y = x * sin + y * cos; }
        }
    }
